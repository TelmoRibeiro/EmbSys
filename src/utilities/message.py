...
def message_packet(msg_ID,msg_flag,msg_timestamp=datetime.datetime.now()):
    return f"{msg_ID}@{msg_timestamp}@{msg_flag}@"
...
def encode_packet(msg_ID,msg_flag,msg_timestamp=datetime.datetime.now()):
    data_send = message_packet(msg_ID,msg_flag,msg_timestamp)
    data_encd = encode(data_send)
    return data_send,data_encd

def decode_packet(data_recv):
    data_decd = decode(data_recv)
    msg_ID,msg_timestamp,msg_flag = message_unpack(data_decd)
    return msg_ID,msg_timestamp,msg_flag