...
'''
    ATTENTION:
        ...
        - THERE ARE SOME "MANDATORY" SECTIONS IN THE FOLLOWING IMPLEMENTATION THAT
            ARE NOT INDEED "MANDATORY"
        - MANY DESIGN CHOICES WERE MADE TO HAVE A NEET DUALITY CONCEPT
...
def yourMainLogic(service):
    ...
    while True:
        ...
        sleep(3)
        data_buff = ["OPEN_E","CLOSE_E","SENSOR_E"]
...
def main():
    mouset_thread = threading.Thread(target=client,args=(network.MOUSET_CLIENT,))
    mouset_thread.start()
    urmain_mouset_thread = threading.Thread(target=yourMainLogic,args=(network.MOUSET_CLIENT,))
    urmain_mouset_thread.start()
    # RUNNING THREADS #
