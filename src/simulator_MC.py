...
from utilities.network import CC_IPV4,MC_PORT
...
def client():
    try:
        ...
        try:
            client_socket.connect((CC_IPV4,MC_PORT))
            log("MC-SIM",f"connection established with {CC_IPV4}")
            try:
                ...
                while True:
                    ...
                    log("MC-SIM",f"sending {data_send}...")
                    ...
                    data_decd = decode(data_recv)
                    msg_ID,msg_timestamp,msg_content = message_unpack(data_decd)
                    log("MC-SIM",f"received: {msg_ID} | {msg_timestamp} | {msg_content}")
                    ####################
                    msg_ID += 1
            except Exception as e:
                log("MC-SIM",f"error: {e}")
        except ConnectionRefusedError:
            log("MC-SIM",f"connection with {CC_IPV4} refused")
        finally:
            client_socket.close()
    except KeyboardInterrupt:
        log("MC-SIM",f"shutting down...")
